...
class ZhejiangSpider(scrapy.Spider):
    name = "zhejiang"
    base_url = "http://www.zj.gov.cn"
    allowed_domains = [base_url]
    start_urls = ['http://www.zj.gov.cn/col/col1554467/index.html?uid=4759812&pageNum=23']

    def parse(self, response):
        html = pq(str(response.body, encoding='utf-8'), parser='html')
        for a in html('.xc_pgContainer li a'):
    ...
    def parse_detail(self, response):
        ...
        publish_time = html('.list li')[0].text.split("：")[1].strip()
    ...
    def parse_next_pate(html):
        return html('.xc_pgNext:not(.xc_pgNextDisabled)')[0].attrib['href']
